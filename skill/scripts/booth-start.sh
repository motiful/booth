...
case "$CMD" in
  start)
    ...
    CLAUDE_CMD="claude --append-system-prompt '${BOOTH_PROMPT}'"
    tmux -L "$SOCKET" send-keys -t "$SESSION" "while true; do ${CLAUDE_CMD}; echo '[booth] DJ exited, restarting in 3s... (Ctrl-C to stop)'; sleep 3; done" Enter
